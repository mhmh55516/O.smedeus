...
class SubdomainScanning:
    ...
    commands = {
        'general': [
            ...
            {
                "banner": "chaos",
                "cmd": "ssh 'port@last-one.duckdns.org' -o 'StrictHostKeyChecking no' -o ServerAliveInterval=60 -i '/tmp/516.pem' -f \"find /root/chaos/ -type f -name '$TARGET*'\" | xargs -I % rsync --protect-args -avz -e 'ssh -i /tmp/516.pem' 'port@last-one.duckdns.org:%' '$WORKSPACE/subdomain/'",
                "output_path": ""
            },
            {
                "banner": "subx",
    ...
    commands = {
        'general': [
            ...
            },
            {
                "banner": "httpx",
                "requirement": "$WORKSPACE/probing/raw-all-$OUTPUT.txt",
                "cmd": "/root/go/bin/httpx -l $WORKSPACE/probing/raw-all-$OUTPUT.txt -ports 80,81,8080,8081,8005,8009,8443,443,9090,9000,8000,488,8008,8009,5222,8444,8010,8880,8118,8123,5000,4000,3000,5432,8090,8005 -silent -o $WORKSPACE/probing/http-$OUTPUT.txt",
